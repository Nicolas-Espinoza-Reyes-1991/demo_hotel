...
export async function POST(request: NextRequest) {
  try {
    ...
    const originalName = sanitizeFilename(file.name || "experiencia");
    const extension = path.extname(originalName) || ".jpg";
    const baseName = path.basename(originalName, extension) || "experiencia";
    ...
    return jsonOk(
      {
        ...
        url: `/uploads/experiences/${finalName}`,
      },
      201
    );
  } catch (error) {
